...
function renderNav() {
  const router = createMemoryRouter(
    ["/", "/projects", "/leaders", "/login", "/member", "/admin", "/apply", "/me"].map((path) => ({
      path,
      element: <Nav />,
    })),
    { initialEntries: ["/"] },
...
describe("Nav", () => {
  ...
  it("로그인 전에는 로그인 링크와 지원하기 CTA가 보이고 로그아웃 · 내 정보는 없다", async () => {
    vi.mocked(getMe).mockRejectedValue({ code: "UNAUTHORIZED", message: "인증이 필요합니다." });
    renderNav();
  ...
  it("ADMIN으로 로그인하면 CTA가 관리자 진입 버튼으로 바뀌고 로그인 링크는 없다", async () => {
  ...
  it("MEMBER로 로그인하면 CTA가 부원 진입 버튼으로 바뀐다", async () => {
    vi.mocked(getMe).mockResolvedValue(meWithRole("MEMBER"));
    renderNav();

    expect(await screen.findByRole("link", { name: "부원" })).toHaveAttribute("href", "/member");
  });

  it("GUEST로 로그인하면 갈 곳이 없어 지원하기 CTA와 로그아웃만 보인다", async () => {
    ...
    expect(screen.queryByRole("link", { name: "부원" })).not.toBeInTheDocument();
  });
